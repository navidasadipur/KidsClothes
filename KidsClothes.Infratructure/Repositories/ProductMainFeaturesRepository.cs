...
namespace KidsClothes.Infrastructure.Repositories
{
    public class ProductMainFeaturesRepository : BaseRepository<ProductMainFeature, MyDbContext>
    {
        ...
        {
            var minPrice = _context.ProductMainFeatures.Where(f => f.IsDeleted == false).Min(f => f.Price);
            return minPrice;
        }

        public long GetMaxPrice()
        {
            var maxPrice = _context.ProductMainFeatures.Where(f => f.IsDeleted == false).Max(f => f.Price);
            return maxPrice;
        }
    }
}
